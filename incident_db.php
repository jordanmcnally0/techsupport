...
function get_assigned_incidents($id) {
    ...
    $query = 'SELECT * FROM incidents i LEFT JOIN customers c
              ON i.customerID = c.customerID WHERE (techID = :id) AND (dateClosed IS NULL or dateClosed < "1900-01-01")';
    try {
        $statement = $db->prepare($query);
...
function get_incidents_by_technician($id) {
    ...
    $query = 'SELECT c.firstName, c.lastName, i.*
              FROM incidents i
                  INNER JOIN customers c ON c.customerID = i.customerID
              WHERE techID = :id AND (dateClosed IS NULL or dateClosed < "1900-01-01")';
    try {
        $statement = $db->prepare($query);
        $statement->bindValue(':id', $id);
        $statement->execute();
...
function add_incident($customer_id, $product_code, $title, $description) {
    global $db;
    $date_opened = date('Y-m-d');  // get current date in dd-mm-yyyy format
    $query =
